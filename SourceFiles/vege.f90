...
PUBLIC INITIALIZE_LEVEL_SET_FIREFRONT,LEVEL_SET_FIREFRONT_PROPAGATION,END_LEVEL_SET,INITIALIZE_RAISED_VEG, &
       ...
       BNDRY_VEG_MASS_ENERGY_TRANSFER,LEVEL_SET_BC,LEVEL_SET_DT
EXTERNAL DLSODA
...
INTEGER, INTENT(IN) :: NM
LOGICAL :: VEG_DEGRADATION_LINEAR,VEG_DEGRADATION_ARRHENIUS,VEG_DEGRADATION_STIFF_SOLVER
...
DOUBLE PRECISION RTOL_VEG,TOUT_VEG,TSTART_VEG
...
! Intialize quantities
 ...
 VEG_DEGRADATION_STIFF_SOLVER = .FALSE.
...
! Thermal degradation approach parameters
 ...
 IF(PC%VEG_DEGRADATION == 'ARRHENIUS STIFF') VEG_DEGRADATION_STIFF_SOLVER = .TRUE.
...
! Stiff ODE solver for Arrhenius thermal degradation

 IF_STIFF_SOLVER: IF (VEG_DEGRADATION_STIFF_SOLVER .AND. .NOT. LP%IGNITOR) THEN
...
!print '(A,2x,3I3)','NEQ,',NEQ_VEG(:)
!print '(A,2x,10ES12.4)','Y,',Y_VEG(:)
!print '(A,2x,1ES12.4)','T,',T
!print '(A,2x,1ES12.4)','DT,',DT
!print '(A,2x,1ES12.4)','TOUT,',TOUT_VEG
!print '(A,2x,I3)','ITOL,',ITOL_VEG

!print '(A,2x,1ES12.4)','RTOL,',RTOL_VEG
!print '(A,2x,6ES12.4)','ATOL,',ATOL_VEG(:)
!print '(A,2x,1I3)','ITASK,',ITASK_VEG
!print '(A,2x,1I3)','-ISTATE,',ISTATE_VEG
!print '(A,2x,1I3)','IOPT,',IOPT_VEG

!print '(A,2x,1I3)','LRW,',LRW_VEG
!print '(A,2x,1I3)','LIW,',LIW_VEG
!print '(A,2x,1I3)','JT,',JT_VEG
...
!F(NM == 1 .AND. II==15 .AND. JJ==11 .AND. KK==6) print '(A,2x,7ES12.4)','DLSODA  Y(1:7),',Y_VEG(1:7)
!F(NM == 1 .AND. IWORK_VEG(19) == 2)print '(A,1x,I3)','I,J,K @ IWORK(19)=2 = ',II,JJ,KK
...
! Dry virgin vegetation
  DMPV_VEG          = LP%VEG_FUEL_MASS - Y_VEG(2)
  MPV_VOLIT         = CHAR_FCTR*DMPV_VEG
  H_SENS_VEG_VOLIT  = 0.0445_EB*(TMP_VEG_NEW**1.5_EB - TMP_GAS**1.5_EB) - 0.136_EB*(TMP_VEG_NEW - TMP_GAS)
  H_SENS_VEG_VOLIT  = H_SENS_VEG_VOLIT*1000._EB !J/kg
  Q_VEG_VOLIT       = MPV_VOLIT*H_SENS_VEG_VOLIT !J
  MW_VEG_VOLIT_TERM = MPV_VOLIT/SPECIES(FUEL_INDEX)%MW

! Char
  MPV_CHAR_LOSS     = (Y_VEG(4) - LP%VEG_ASH_MASS)/NU_ASH_VEG
  MPV_CHAR_CO2      = (1._EB + NU_O2_CHAR_VEG - NU_ASH_VEG)*MPV_CHAR_LOSS
  Q_VEG_CHAR        = MPV_CHAR_LOSS*H_CHAR_VEG 
  Q_VEG_CHAR_TOTAL  = Q_VEG_CHAR_TOTAL + Q_VEG_CHAR
...
! Update mass per volume values
  LP%VEG_MOIST_MASS = Y_VEG(1)
  LP%VEG_FUEL_MASS  = Y_VEG(2)
  LP%VEG_CHAR_MASS  = Y_VEG(3)
  LP%VEG_ASH_MASS   = Y_VEG(4)

 ENDIF IF_STIFF_SOLVER
...
!***********************************************************************************************
!
! Subroutine called by stiff ODE solver DLSODA
!
! NEQ(1) = number of ODEs
! NEQ(2) = number of computational mesh 
! NEQ(3) = particle number 
! Y(1) = Moisture mass
! Y(2) = Dry veg mass
! Y(3) = Char mass
! Y(4) = Ash mass
...
DT_OUTPUT_LS  = 0.5_EB
IF (DT_SLCF > 0._EB) DT_OUTPUT_LS = DT_SLCF
