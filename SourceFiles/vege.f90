...
REAL(EB) :: COSDPHIU,DPHIDX,DPHIDY,DPHIDOTU,XI,YJ,ZK,RCP_GAS,TE_HRR,TE_HRR_TOTAL
...
!   TE_HRR_TOTAL  = TE_HRR_TOTAL + TE_TIME_FACTOR*LP%LSET_HRRPUV*DX(II)*DY(JJ)*DZ(KK)
    TE_HRR =  TE_TIME_FACTOR*LP%LSET_HRRPUV*DX(II)*DY(JJ)*DZ(KK)
    IF (T_CFD - LP%T > PC%TE_BURNTIME) TE_HRR = 0.0_EB
    Q(II,JJ,KK) = Q(II,JJ,KK) + TE_HRR
!   D_LAGRANGIAN(II,JJ,KK) = D_LAGRANGIAN(II,JJ,KK)  +  &
