...
IMPLICIT NONE
PRIVATE
PUBLIC INITIALIZE_LEVEL_SET_FIRESPREAD,LEVEL_SET_FIRESPREAD,END_LS,INITIALIZE_RAISED_VEG, &
...
!For Level Set
INTEGER  :: LIMITER_LS,LU_FLI_LS,LU_ROSX_LS,LU_ROSY_LS,LU_SLCF_LS,LU_TOA_LS,NX_LS,NY_LS
...
REAL(EB) :: CP_MOIST_AND_VEG,DZVEG_L,ETAVEG_H,H_CONV_L, &
            ...
            TMP_BOIL,TMP_G,DTMP_L,RE_H,RE_VEG_PART,U2,V2,RE_D,Y_O2,ZVEG
...
REAL(EB) :: CP_ASH,CP_CHAR,CP_H2O,CP_VEG,CP_TOTAL,DTMP_VEG,H_VAP_H2O,TMP_VEG,TMP_VEG_NEW
...
!  Minimum bound on dry veg.Newer, linear or Arrhenius degradation and char
  ...
  MPA_CHAR_MIN  = SF%VEG_CHARFRAC*MPA_VEG_MIN !kg/m^2
...
! Find top of vegetation which burns downward from the top
  IF (SF%VEG_CHAR_OXIDATION) THEN
    DO IVEG_L = 1,NVEG_L 
      IF(WC%VEG_CHARMASS_L(IVEG_L) <= MPA_CHAR_MIN) LBURN = IVEG_L
    ENDDO
  ELSE
    DO IVEG_L = 1,NVEG_L 
...
!   IF(H_CYLINDER_RE) THEN 
    IF(SF%VEG_HCONV_CYLRE) THEN 
     ...
     RE_VEG_PART = 4._EB*RHO_GAS*SQRT(U2 + V2 + W(IIG,JJG,1)**2)/SF%VEG_SVRATIO/MU_AIR
     RE_H = RE_VEG_PART**0.466_EB
     H_CONV_L = 0.25*0.683*SF%VEG_SVRATIO*K_AIR*RE_H !W/m^2
    ENDIF
!
...
!combustion)
      IF_CHAR_OXIDATION: IF (SF%VEG_CHAR_OXIDATION .AND. MPA_CHAR > 0.0_EB) THEN
         RE_D = RHO_GAS*SQRT(U2 + V2 + W(IIG,JJG,1)**2)*2._EB/SF%VEG_SVRATIO/MU_AIR 
         ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
         CALL GET_MASS_FRACTION(ZZ_GET,O2_INDEX,Y_O2)
         MPA_CHAR_LOSS = DT*RHO_GAS*Y_O2*A_CHAR_VEG/NU_O2_CHAR_VEG*SF%VEG_SVRATIO*  &
...
ALLOCATE(ROS_X_OUT(NX_LS,NY_LS)); CALL ChkMemErr('VEGE:LEVEL SET','LSROSX',IZERO)
ROS_X_OUT = -1.0
ALLOCATE(ROS_Y_OUT(NX_LS,NY_LS)); CALL ChkMemErr('VEGE:LEVEL SET','LSROSY',IZERO)
ROS_Y_OUT = -1.0

! Fire line intensity, kW/m 
ALLOCATE(FLI_OUT(NX_LS,NY_LS)); CALL ChkMemErr('VEGE:LEVEL SET','LSFLI',IZERO)
FLI_OUT = -1.0
...
ALLOCATE(PHI_W_X(NX_LS,NY_LS)) ; CALL ChkMemErr('VEGE:LEVEL SET','PHI_W_X',IZERO)
ALLOCATE(PHI_W_Y(NX_LS,NY_LS)) ; CALL ChkMemErr('VEGE:LEVEL SET','PHI_W_Y',IZERO)
ALLOCATE(PHI_W(NX_LS,NY_LS))   ; CALL ChkMemErr('VEGE:LEVEL SET','PHI_W',IZERO)
ALLOCATE(UMF_X(NX_LS,NY_LS))   ; CALL ChkMemErr('VEGE:LEVEL SET','UMF_X',IZERO)
ALLOCATE(UMF_Y(NX_LS,NY_LS))   ; CALL ChkMemErr('VEGE:LEVEL SET','UMF_Y',IZERO)
PHI_WS    = 0.0_EB
...
!Find slope factors for use in Rothermel ROS model
    CALL ROTH_SLOPE_COEFF(NM,IIG,JJG,SF%VEG_LSET_BETA)

!Find wind, combined wind & slope, and midflame windspeed factors for use in Rothermel ROS model and
! ellipse fireline model
    CALL ROTH_WINDANDSLOPE_COEFF(NM,IIG,JJG,KKG,SF%VEG_LSET_BETA,SF%VEG_LSET_HT,SF%VEG_LSET_SIGMA)
...
!************************************************************************************************
SUBROUTINE ROTH_WINDANDSLOPE_COEFF(NM,I,J,K,VEG_BETA,VEG_HT,VEG_SIGMA)
!************************************************************************************************
!
! Compute components and magnitude of the wind coefficient vector and the combined
! wind and slope coefficient vectors that are used in the Rothermel surface fire spread rate
! formula to obtain the local surface fire ROS from the zero wind, zero slope,
! ROS that's in the input file

INTEGER,  INTENT(IN) :: I,J,K,NM
REAL(EB), INTENT(IN) :: VEG_BETA,VEG_HT,VEG_SIGMA
INTEGER :: KDUM,KSTART,KWIND
REAL(EB) :: PHX,PHY,MAG_PHI

!Find k array index corresponding to ~6.1 m AGL height for to determine midflame wind
KWIND = 0
KDUM  = K
...
PHI_W(I,J) =  SQRT(PHI_W_X(I,J)**2 + PHI_W_Y(I,J)**2) !used in LS paper
...
UMF(I,J) = SQRT(UMF_X(I,J)**2 + UMF_Y(I,J)**2) !used in LS paper
...
END SUBROUTINE ROTH_WINDANDSLOPE_COEFF
...
REAL(EB) :: FLI,HEAD_WIDTH_FCTR,IGNITION_WIDTH_Y,ROS_FLANK1,TIME_LS_LAST,BURNOUT_FCTR,R_BURNOUT_FCTR, &
            BURNOUT_TIME
REAL(EB) :: PHI_CHECK
REAL(FB) :: TIME_LS_OUT
...
!------------------------------------------------------------------------------------------------
 ...
 WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
  ...
  SF  => SURFACE(WC%SURF_INDEX)
! IF (.NOT. SF%VEG_LSET_SPREAD .AND. .NOT. SF%VEG_LSET_IGNITE) CYCLE WALL_CELL_LOOP
  IIG = WC%IIG
  JJG = WC%JJG
  

! Ignite landscape at user specified location(s) and time(s)
  IF (SF%VEG_LSET_IGNITE_T > 0.0_EB .AND. SF%VEG_LSET_IGNITE_T < DT_LS) THEN
    PHI_LS(IIG,JJG) = PHI_MAX_LS 
    WC%QCONF = 0._EB
  ENDIF
  IF (SF%VEG_LSET_IGNITE_T >= TIME_LS .AND. SF%VEG_LSET_IGNITE_T <= TIME_LS + DT_LS) THEN 
    PHI_LS(IIG,JJG) = PHI_MAX_LS 
    WC%QCONF = 0._EB
  ENDIF
...
!   IF (PHI_LS(IIG,JJG) == PHI_MAX_LS) WC%ONE_D%TMP_F = 373._EB
    WC%QCONF = 0._EB
! print*,'hc',rn%heat_of_combustion
! print*,'charfrac',sf%veg_charfrac
! print*,'w',sf%veg_load
! print*,'sv',sf%veg_svratio
! print*,'r_bo',r_burnout_fctr
!   WC%QCONF = RN%HEAT_OF_COMBUSTION*(1.0_EB-SF%VEG_CHARFRAC)*SF%VEG_LOAD*SF%VEG_SVRATIO*R_BURNOUT_FCTR
    ...
    IF (PHI_LS(IIG,JJG) >= -SF%VEG_LSET_PHIDEPTH .AND. BURN_TIME_LS(IIG,JJG) <= BURNOUT_TIME) THEN 
     WC%QCONF = -SF%VEG_LSET_HEAT_OF_COMBUSTION*(1.0_EB-SF%VEG_CHARFRAC)*SF%VEG_LOAD*100._EB* &
                  SF%VEG_LSET_SIGMA*R_BURNOUT_FCTR
     WC%VEG_HEIGHT = SF%VEG_HEIGHT*(1._EB - BURN_TIME_LS(IIG,JJG)/BURNOUT_TIME)
...
! Save Time of Arrival (TOA), Rate of Spread components, and Fireline Intensity 

 DO IDUM=1,NX_LS
     DO JDUM=1,NY_LS
         IF (PHI_LS(IDUM,JDUM)>=0._EB .AND. TOA(IDUM,JDUM)<=-1.0_EB) THEN 
          ...
          FLI = SQRT(SR_Y_LS(IDUM,JDUM)**2 + SR_X_LS(IDUM,JDUM)**2)*0.001_EB* &
                  SF%VEG_LSET_HEAT_OF_COMBUSTION*(1.0_EB-SF%VEG_CHARFRAC)*SF%VEG_LOAD
...
!       XSF = ( DPHIDY / MAG_F ) ! in LS paper
!       YSF = (-DPHIDX / MAG_F)  ! in LS paper
   ...
   IF (LSET_ELLIPSE) THEN
       ...
       ROS_TMP = ROS_HEAD(I,J) * (1.0_EB + PHI_WS(I,J)) !used in LS paper
!      ROS_TMP = ROS_HEAD(I,J) * (1.0_EB + PHI_W(I,J) + PHI_S(I,J)) !spreads faster than LS paper
       
       !Mag of wind speed at midflame ht must be in units of m/s here   
       UMF_DUM = UMF(I,J)/60.0_EB
!      UMF_X_DUM = UMF_X(I,J)/60.0_EB
!      UMF_Y_DUM = UMF_X(I,J)/60.0_EB
!      UMF_DUM   = UMF_X(I,J)*NORMAL_FIRELINE(1) + UMF_Y(I,J)*NORMAL_FIRELINE(2)
       ...
       ! A_ELPS and B_ELPS are *opposite* in notation from Farsite and Richards this is in LS paper
...
!      C_ELPS =  A_ELPS - (ROS_TMP/HB)
       ...
       ! Denominator used in spread rate equation from Richards 1990 (also in Farsite) this is in LS paper
!      DENOM = B_ELPS2 * (YSF * COS_THETA + XSF * SIN_THETA)**2 + &
!               A_ELPS2 * (XSF * COS_THETA - YSF * SIN_THETA)**2

       ! Denominator used in spread rate equation from Richards 1990 (in Farsite it's different) 
...
!  
!This is with A_ELPS2 and B_ELPS2 notation consistent with Finney and Richards
